...
def my_profile():
    ...
    if request.method == "POST":
        ...
        if len(orders) == 0:
            no_items = True
        return render_template("my_profile.html", no_items=no_items, rows = rows, orders = orders, first_name = first_name, last_name = last_name, email = email)
...
def confirm_order():
    ...
    if request.method == "POST":
        ...
        db.execute("DELETE FROM user_cart WHERE user_id = :user_id",
                       user_id=session["user_id"])
        return render_template("confirm_order.html", total=total, items=items, cart_id=cart_id)
